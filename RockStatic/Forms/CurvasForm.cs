    ...
    public partial class CurvasForm : Form
    {
        ...
        public void Estimar()
        {
            ...
            if (padre.actual.phantomEnDicom)
            {
                ...
                padre.actual.datacuboLow = new MyDataCube();
                padre.actual.datacuboLow.dataCube = new List<MyDicom>();

                for (int i = 0; i < padre.actual.datacuboHigh.dataCube.Count; i++)
                {
                    // se leen uno a uno los dicom de baja para ahorrar memoria
                    MyDicom dicom = new MyDicom(padre.actual.folderLow + "\\" + i);
                    padre.actual.datacuboLow.dataCube.Add(new MyDicom());
                    
                    // segementacion del core
                    AuxThread thread = new AuxThread(dicom.pixelData, padre.actual.areaCore.x, padre.actual.areaCore.y, padre.actual.areaCore.width, dicom.selector.Columns.Data, dicom.selector.Rows.Data, new System.Threading.ManualResetEvent(false));
                    padre.actual.datacuboLow.dataCube[i].segCore = thread.SegmentarCircular();

                    // segmentacion del Phantom1
                    thread = new AuxThread(dicom.pixelData, padre.actual.areaPhantom1.x, padre.actual.areaPhantom1.y, padre.actual.areaPhantom1.width, dicom.selector.Columns.Data, dicom.selector.Rows.Data, new System.Threading.ManualResetEvent(false));
                    padre.actual.datacuboLow.dataCube[i].segPhantom1 = thread.SegmentarCircular();

                    // segmentacion del Phantom2
                    thread = new AuxThread(dicom.pixelData, padre.actual.areaPhantom2.x, padre.actual.areaPhantom2.y, padre.actual.areaPhantom2.width, dicom.selector.Columns.Data, dicom.selector.Rows.Data, new System.Threading.ManualResetEvent(false));
                    padre.actual.datacuboLow.dataCube[i].segPhantom2 = thread.SegmentarCircular();

                    // segmentacion del Phantom1
                    thread = new AuxThread(dicom.pixelData, padre.actual.areaPhantom3.x, padre.actual.areaPhantom3.y, padre.actual.areaPhantom3.width, dicom.selector.Columns.Data, dicom.selector.Rows.Data, new System.Threading.ManualResetEvent(false));
                    padre.actual.datacuboLow.dataCube[i].segPhantom3 = thread.SegmentarCircular();
                }
